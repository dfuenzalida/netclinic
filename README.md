...
* A PostgreSQL server, configured with the database schema and data on startup
* An NGINX Reverse Proxy configured to have the front-end and backend accessible from the same port
* The [adminer database manager](https://www.adminer.org/en/) on port 8088 to inspect the database

Start the backend API with: `dotnet run --environment Development` from the `NetClinic.Api` directory. You can validate that the API is working by opening `http://localhost:5299/api/vets` and the veterinarian data as JSON. 
